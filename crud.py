...
def read():
	try:
		...
		usuarios_lista = cursor.fetchone()
		nombre.set(usuarios_lista[1])
		password.set(usuarios_lista[2])
		apellido.set(usuarios_lista[3])
		direccion.set(usuarios_lista[4])
		comentario_text.insert(1.0,usuarios_lista[5]) 
		conexion.commit()
		conexion.close()
	except ValueError:
		messagebox.showwarning("ERROR","valor no admitido")
	except TypeError:
		messagebox.showwarning("ERROR","el registro no existe en la base de datos")
#------------------------------------- UPDATE ------------------------------------------
def update():
	global indice
	index = int(indice.get())
	...
	conexion = sqlite3.connect("base.db")
	cursor = conexion.cursor()
	cursor.execute('''UPDATE usuarios SET NOMBRE=?,CONTRASEÑA=?,APELLIDO=?,DIRECCION=?,COMENTARIO=? WHERE ID=?''',(nombre.get(),password.get(),apellido.get(),direccion.get(),comentario,index)) 
	messagebox.showinfo("Confirmacion","registro actualizado")
	conexion.commit()
	conexion.close
	borrar()
#------------------------------------- DELETE ------------------------------------------
def delete():
	try:
		...
		conexion = sqlite3.connect("base.db")
		cursor = conexion.cursor()
		cursor.execute("DELETE FROM usuarios WHERE ID='%s'"% index)
		messagebox.showinfo("Confirmacion","registro borrado")
		conexion.commit()
		conexion.close()
		borrar()
	except ValueError:
...
comentario_text.grid(row=5,column=1,padx=10,pady=10,columnspan=3)
